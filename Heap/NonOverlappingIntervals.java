...
public class NonOverlappingIntervals {
    public int eraseOverlapIntervals(int[][] intervals) {
        ...
        // [-73, -26][-65, -11] [-63, 2][-62, -49][-52, 31][-40, -26][-31, 49][30,
        // 47][58, 95][66, 98][82, 97][95, 99]
        ...
        // add first end time
        minHeap.add(intervals[0][1]); // 2
        int count = 0;
        for (int i = 1; i < intervals.length; i++) {
            int currentStartTime = intervals[i][0]; // 1
            int currentEndTime = intervals[i][1]; // 1
            ...
            if (currentStartTime < lastEndTime) {
                ...
                // good
                minHeap.poll();
                minHeap.add(intervals[i][1]); // current ends
            }
